  ...
  return (
    <div ref={containerRef} className="relative flex-1 overflow-hidden min-w-0 pr-1">
      <span
        ref={textRef}
        style={{
          transform: isHovered && overflowWidth > 0 ? `translateX(-${overflowWidth + 8}px)` : 'translateX(0px)',
          transition: isHovered && overflowWidth > 0 ? `transform ${duration}s linear` : 'transform 0.25s ease-out',
        }}
        className="inline-block whitespace-nowrap text-[13.5px] leading-snug select-none"
      >
        {title || 'New chat'}
      </span>
    </div>
...
export function Sidebar({
  ...
  const displayName = user?.user_metadata?.full_name || user?.user_metadata?.name || user?.email?.split('@')[0] || 'Ayan Ali';
  const planDisplay = userPlan ? userPlan.charAt(0).toUpperCase() + userPlan.slice(1) : 'Go';
  const userEmail = user?.email || 'pathanayan8347@gmail.com';

  /**
   * Account Popover Menu Items
   */
  const renderAccountMenuItems = () => (
    <>
      {/* Account Switcher Submenu (Image 2) */}
      <DropdownMenuSub>
        <DropdownMenuSubTrigger className="flex items-center gap-2.5 px-3 py-2 text-sm rounded-xl">
          ...
          <div className="flex-1 min-w-0 text-left">
            <p className="text-sm font-medium truncate text-foreground leading-tight">{displayName}</p>
            <p className="text-xs text-muted-foreground leading-tight">{planDisplay}</p>
          </div>
        </DropdownMenuSubTrigger>
        <DropdownMenuSubContent alignOffset={-80} sideOffset={6} className="w-64 rounded-2xl p-1.5">
          ...
          <DropdownMenuItem asChild>
            <Link href="/auth/login" className="flex items-center gap-2.5 px-3 py-2 cursor-pointer rounded-xl text-sm">
      ...
      {/* Upgrade plan */}
      <DropdownMenuItem asChild>
        <Link href="/upgrade" className="flex items-center gap-2.5 px-3 py-2 cursor-pointer rounded-xl">
      ...
      {/* Personalization */}
      <DropdownMenuItem asChild>
        <Link href="/settings" className="flex items-center gap-2.5 px-3 py-2 cursor-pointer rounded-xl">
      ...
      <DropdownMenuSub>
        ...
        <DropdownMenuSubContent alignOffset={-80} sideOffset={6} className="w-40 rounded-2xl p-1.5">
      ...
      {/* Help Submenu with full items (opens upwards so Report a bug aligns with Help) */}
      <DropdownMenuSub>
        <DropdownMenuSubTrigger className="flex items-center gap-2.5 px-3 py-2 text-sm rounded-xl">
          <LifeBuoy className="w-4 h-4 text-muted-foreground" />
          <span>Help</span>
        </DropdownMenuSubTrigger>
        <DropdownMenuSubContent alignOffset={-235} sideOffset={6} className="w-56 rounded-2xl p-1.5">
          <DropdownMenuItem asChild>
            <Link href="/support/help" className="flex items-center gap-2.5 px-3 py-2 cursor-pointer rounded-xl text-sm">
              <HelpCircle className="w-4 h-4 text-muted-foreground" />
              <span>Help center</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild>
            <Link href="/company/blog" className="flex items-center gap-2.5 px-3 py-2 cursor-pointer rounded-xl text-sm">
              <PenLine className="w-4 h-4 text-muted-foreground" />
              <span>Release notes</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild>
            <Link href="/product/docs" className="flex items-center gap-2.5 px-3 py-2 cursor-pointer rounded-xl text-sm">
              <ArrowDownCircle className="w-4 h-4 text-muted-foreground" />
              <span>Download apps</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild>
            <Link href="/settings" className="flex items-center gap-2.5 px-3 py-2 cursor-pointer rounded-xl text-sm">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem asChild>
            <Link href="/support/terms" className="flex items-center gap-2.5 px-3 py-2 cursor-pointer rounded-xl text-sm">
              <FileText className="w-4 h-4 text-muted-foreground" />
              <span>Terms of Service</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild>
            <Link href="/support/privacy" className="flex items-center gap-2.5 px-3 py-2 cursor-pointer rounded-xl text-sm">
              <Info className="w-4 h-4 text-muted-foreground" />
              <span>Privacy Policy</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild>
            <Link href="/company/contact" className="flex items-center gap-2.5 px-3 py-2 cursor-pointer rounded-xl text-sm">
  ...
  // ----------------------------------------------------
  // Collapsed Mini Sidebar (56px Rail)
  // ----------------------------------------------------
  if (!isOpen) {
    return (
      <div className="w-[56px] h-screen bg-sidebar border-r border-border flex flex-col items-center py-3.5 justify-between shrink-0 select-none z-30">
        <div className="flex flex-col items-center gap-2">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                onClick={onNewChat}
                className="w-10 h-10 rounded-xl flex items-center justify-center text-mutedforeground hover:text-foreground hover:bg-secondary transition-colors cursor-pointer"
                aria-label="New chat"
              >
  ...
  return (
    <>
      ...
      <aside className="fixed md:static inset-y-0 left-0 z-50 w-[260px] h-screen bg-sidebar border-r border-border/80 flex flex-col shrink-0 select-none">
        {/* Top Header */}
        <div className="p-3 pb-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-0.5">
            <Tooltip>
              ...
              <TooltipContent className="text-xs">Search chats</TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              ...
              <TooltipContent className="text-xs">Close sidebar</TooltipContent>
            </Tooltip>
          </div>
        </div>
        ...
        <div className="px-3 pt-1 pb-2 space-y-2">
          <button
            onClick={onNewChat}
            className="w-full flex items-center justify-between h-10 px-3 rounded-xl border border-border/70 bg-background hover:bg-secondary text-foreground text-sm font-medium group cursor-pointer transition-all duration-150 active:scale-[0.99]"
          >
            ...
            {Object.entries(groupedChats).map(([group, groupChats]) => {
              ...
              return (
                <div key={group} className="space-y-0.5">
                  <div className="px-3 py-1 text-[11px] font-semibold tracking-wider text-muted-foreground/80 uppercase">
                    {group}
                  </div>
                  {groupChats.map((chat) => {
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          'group relative flex items-center justify-between px-3 py-2 rounded-xl text-sm cursor-pointer transition-all duration-150',
                          isSelected
                      ...
                      >
                        ...
                        <div
                          className={cn(
                            'absolute right-2 flex items-center gap-1 bg-gradient-to-l from-secondary via-secondary to-transparent pl-3 py-0.5 rounded-r-xl transition-opacity duration-150',
                        ...
                        >
                          <Tooltip>
                            <TooltipTrigger asChild>
                              ...
                              >
                                <Star className={cn('w-3.5 h-3.5', chat.starred && 'fill-foreground text-foreground')} />
                              </button>
                            </TooltipTrigger>
        ...
        <div className="p-2 border-t border-border/80 mt-auto">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
            ...
            <DropdownMenuContent
              ...
              align="start"
              sideOffset={8}
              className="w-[244px] rounded-2xl p-1.5 mb-1 bg-popover/98 backdrop-blur-2xl border border-border/80 outline-none focus:outline-none ring-0"
            >
              {renderAccountMenuItems()}
            </DropdownMenuContent>
          </DropdownMenu>
