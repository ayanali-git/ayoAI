...
class ChatGPTModelSelector extends HTMLElement {
  static observedAttributes = ['value', 'model-name', 'placement'];
  ...
  #index = 4;          // committed stop index (0–4), default Ultra
  ...
  constructor() {
    ...
    <style>
      ...
      :host-context(.dark) {
        ...
        --track: #303030;
        --pill-bg: #212121;
        --pill-bg-hover: #2f2f2f;
        --card: #1e1e1e;
        --hairline: #2e2e2e;
      }
      :host(.dark) .popover {
        border: 1px solid rgba(255, 255, 255, 0.1);
        box-shadow: 0 0 0 1px rgba(255, 255, 255, 0.05), 0 8px 32px rgba(0, 0, 0, 0.6);
      }
      :host(.dark) .pill {
        border: 1px solid rgba(255, 255, 255, 0.08);
      }
      :host(.dark) .back-btn:hover {
        background: #2a2a2a;
        color: var(--ink);
      }
      :host(.dark) .adv-chip:hover {
        background: #2a2a2a;
        color: #ececec;
      }
      :host(.dark) .knob {
      ...
      * { box-sizing: border-box; }
      ...
      :host([placement="bottom"]) .popover.is-open {
        translate: 0 0;
      }
      .popover.is-open {
        opacity: 1;
      ...
      :host(.dark) .row:hover { background: #2a2a2a; }
  ...
  connectedCallback() {
    ...
    const updateTheme = () => {
      ...
      this.classList.toggle('dark', Boolean(isDark));
    };
    updateTheme();
  ...
  #bind() {
    ...
    const effortRow = this.shadowRoot.querySelector('[data-row="effort"]');
    if (effortRow) {
      effortRow.addEventListener('click', () => this.#switchView('advanced'));
    }
    const modelRow = this.shadowRoot.querySelector('[data-row="model"]');
    if (modelRow) {
    ...
    const speedRow = this.shadowRoot.querySelector('[data-row="speed"]');
    if (speedRow) {
      speedRow.addEventListener('click', () => {
        const v = speedRow.querySelector('.v');
        if (v) {
          v.textContent = v.textContent === 'Fast' ? 'Standard' : 'Fast';
        }
      });
    }
...
if (typeof customElements !== 'undefined' && !customElements.get('chatgpt-model-selector')) {
  customElements.define('chatgpt-model-selector', ChatGPTModelSelector);
}

export type ChatGPTModelSelectorProps = {
  modelName?: string;
  value?: number;
  className?: string;
  placement?: "top" | "bottom";
  onChange?: (detail: { index: number; tier: string; model: string }) => void;
};

const ChatGPTModelSelectorComponent = React.forwardRef<
  HTMLElement,
...
>(function ChatGPTModelSelectorComponent(
  ...
  return React.createElement("chatgpt-model-selector", {
    ref,
    className,
...
export { ChatGPTModelSelector };
export default ChatGPTModelSelectorComponent;
